...
class Game extends Table
{
	...
	public function __construct()
	{
		...
		// Clear log
		$notifs = $this->getCollectionFromDb("SELECT gamelog_move_id, gamelog_notification FROM gamelog");
		foreach ($notifs as $notif) {
			$logs = json_decode($notif["gamelog_notification"], true);
			foreach ($logs as $log) {
				if ($log["type"] === "cancelNotifs") {
					$this->dbQuery("DELETE FROM gamelog WHERE gamelog_move_id = {$notif['gamelog_move_id']}");
					break;
				}
			}
		}
	...
	protected function setupNewGame($players, $options = []): string
	{
		...
		$this->playerStats->init("placed_camps_points", 0);
		...
		$this->playerStats->init("placed_souvenirs_points", 0);
