...
trait UndoMethods
{
	...
	function undoActionGift(int $active_player_id, array $log): string {
		...
		switch ($gift_type) {
			...
			default: throw new BgaVisibleSystemException('Action.php [actActionGift] Not valid gift type');
		}
		// Notify client
	...
	function undoCamp(int $active_player_id, array $log): string {
		...
		switch ($camp) {
			...
			case 11:
				$this->playerScore->inc($active_player_id, -1);
				$this->playerStats->inc("placed_camps_points", -1, $active_player_id);
				break;
			case 12:
				$this->playerScore->inc($active_player_id, -3);
				$this->playerStats->inc("placed_camps_points", -3, $active_player_id);
				break;
			case 13:
				$this->playerScore->inc($active_player_id, -7);
				$this->playerStats->inc("placed_camps_points", -7, $active_player_id);
				break;
		}
		// Notify client
		if ($best_traveller !== 0) {
