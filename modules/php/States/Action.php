...
class Action extends GameState
{
	...
	public function getArgs(int $active_player_id): array
	{
		...
		// Get travels in hand
		$args["travels"] = $this->game->getTravelsFromHand($active_player_id);
		// Get possible actions
		$travels = array_intersect($args["travels"], $args["used_travels"]);
	...
	function onEnteringState(int $active_player_id, array $args): ?string
	{
		...
		$travels = array_diff($args["travels"], $args["used_travels"]);
	...
	{
		// Check action possibility
		if (!in_array($travel, $args['travels']) || in_array($travel, $args['used_travels']))
	...
	{
		// Check action possibility
		if (!in_array($travel_1, $args['travels']) || in_array($travel_1, $args['used_travels']) || !in_array($travel_2, $args['travels']) || in_array($travel_2, $args['used_travels']))
			throw new BgaUserException('Action.php [actActionTravel] Invalid card choice');
	...
	public function actActionBonus(int $active_player_id, array $args, int $type): string
	{
		...
		else throw new BgaVisibleSystemException("Action.php [actActionTravel] - This action is not possible: $type");
	}
