...
class Souvenir extends GameState
{
	...
	public function actSouvenir(int $active_player_id, array $args, int $postcard, int $space): string
	{
		...
		switch ($bonus) {
			case SouvenirBonusType::Move:
				$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and gains 2 Movement effects'), [
					"player_id" => $active_player_id,
					"postcard" => $postcard,
					"location" => $space,
				]);
				$this->game->moveBonusCounter->inc(2);
				break;
			case SouvenirBonusType::Postcard:
				$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and gains a Postcard effect'), [
					"player_id" => $active_player_id,
					"postcard" => $postcard,
					"location" => $space,
				]);
				...
				break;
			case SouvenirBonusType::Camp:
				$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and gains a Camp effect'), [
					"player_id" => $active_player_id,
					"postcard" => $postcard,
					"location" => $space,
				]);
				...
				break;
			case SouvenirBonusType::Stamp:
				$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and gains a Stamp effect'), [
					"player_id" => $active_player_id,
					"postcard" => $postcard,
					"location" => $space,
				]);
				...
				break;
			case SouvenirBonusType::Travel:
				if ($this->game->travelBonusCounter->get() !== 2) {
					$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and gains an extra Travel card play'), [
						"player_id" => $active_player_id,
						"postcard" => $postcard,
						"location" => $space,
					]);
					$this->game->travelBonusCounter->inc(1);
				} else {
				...
				break;
			case SouvenirBonusType::Point:
				$this->notify->all("souvenir", clienttranslate('${player_name} places a Souvenir and scores 2 points'), [
					"player_id" => $active_player_id,
					"postcard" => $postcard,
					"location" => $space,
				]);
				$this->playerScore->inc($active_player_id, 2);
				$this->playerStats->inc("placed_souvenirs_points", 2, $active_player_id);
				break;
		}
